...
function Header() {
  return (
    <header className="flex flex-row">
      <h1 className="text-2xl flex-1 font-bold py-3">Services</h1>
    </header>
  );
}
...
function ServiceDetails({ service }: { service: ServiceInfo }) {
  return (
    <section className="flex flex-col gap-4">
      <h2 className="text-xl flex-1 font-bold py-3">{service.name}</h2>

      <div className="w-full flex flex-row">
        <img alt={service.name} className="flex-1 border" src={service.image} />
      </div>
